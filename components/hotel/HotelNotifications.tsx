...
interface HotelNotificationsProps {
  onBack: () => void;
}
...
export function HotelNotifications({ onBack }: HotelNotificationsProps) {
  ...
  const loadNotifications = async () => {
    setLoading(true);
    try {
      const response = await hotelAPI.getNotifications();
  ...
  const deleteNotification = async (id: number) => {
    try {
      await hotelAPI.deleteNotification(id);
      await loadNotifications();
    } catch (error: any) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Hotel Notifications</Text>
